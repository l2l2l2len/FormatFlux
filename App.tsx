...
const VisitCounter: React.FC = () => {
  ...
  return (
    <div className={`flex items-center gap-2 text-xs font-medium glass px-4 py-2 rounded-full ${
      isDark ? 'text-text-muted' : 'text-slate'
    }`}>
      <Users size={12} />
...
const BentoCard: React.FC<BentoCardProps> = ({ icon, title, description, lightBg, darkBg, delay = '0s' }) => {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  ...
  return (
    <div
      ref={cardRef}
      className={`rounded-3xl p-8 transition-all duration-700 hover:scale-[1.02] ${
        isDark ? `${darkBg} hover:shadow-glow` : `${lightBg} shadow-soft hover:shadow-lg`
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          <BentoCard
            ...
            lightBg="bg-bento-rose"
            darkBg="bg-muted-rose"
            delay="0s"
          />
          <BentoCard
            ...
            lightBg="bg-bento-sage"
            darkBg="bg-muted-sage"
            delay="0.15s"
          />
          <BentoCard
            ...
            lightBg="bg-bento-indigo"
            darkBg="bg-muted-indigo"
            delay="0.3s"
          />
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className="relative inline-block mb-6">
        <div className={`w-20 h-20 rounded-2xl flex items-center justify-center mx-auto group transition-shadow duration-300 ${
          isDark ? 'glass-strong hover:shadow-glow' : 'bg-white shadow-soft hover:shadow-lg'
        }`}>
          {icon}
        </div>
...
const Footer: React.FC = () => {
  ...
  return (
    <footer className={`py-12 mt-auto ${
      isDark ? 'glass' : 'bg-paper-dark/30 border-t border-ink/5'
    }`}>
...
const HeroSection: React.FC = () => {
  ...
  return (
    <div className="text-center space-y-8 mb-16 max-w-3xl mx-auto pt-8">
      <div
        className={`inline-block px-4 py-2 rounded-full glass font-medium text-sm tracking-wide opacity-0 animate-fade-up ${
          isDark ? 'text-text-secondary' : 'text-ink-light'
        }`}
      >
      ...
      <h1
        className={`font-serif text-5xl md:text-7xl font-bold tracking-tight leading-[1.1] opacity-0 animate-fade-up-delay ${
          isDark ? 'text-text-primary' : 'text-ink'
        }`}
      >
        Universal{' '}
      ...
      <p
        className={`text-lg md:text-xl font-medium max-w-xl mx-auto leading-relaxed opacity-0 animate-fade-up-delay-2 ${
          isDark ? 'text-text-secondary' : 'text-ink-light'
        }`}
      >
...
const AppContent: React.FC = () => {
  ...
  return (
    <ToastProvider>
      <div className={`min-h-screen flex flex-col font-sans selection:bg-accent-indigo selection:text-white transition-colors duration-300 ${
        isDark ? 'bg-midnight text-text-primary' : 'bg-paper text-ink'
      }`}>
        {/* Animated Background */}
        <BackgroundWaves />
